...
| 三平台文件集成与发行交付 | 已实现 | Windows 未签名便携包带 `unsigned` 文件名标识，也支持 Artifact Signing/PFX 签名与时间戳验证；macOS 文档类型、Linux desktop/MIME、系统废纸篓和三平台 GitHub Release 包均已落地；应用不检测、下载或自行安装更新，“帮助 → 更新”只打开 GitHub Releases |
