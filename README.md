...
“设置 → 高级 → 应用图标”提供四款带预览的内置图标，默认使用 A 软萌立体原版。选择后立即预览，保存后记住选择，取消恢复原图标。Windows 更新运行窗口图标，macOS 更新 Dock 图标，Linux 使用窗口图标及安装器注册的桌面入口；桌面环境可能缓存图标，安装包、文件管理器和已固定快捷方式仍保留默认图标。
...
仅在窗口处于前台时监测当前查看的日志，通过系统通知刷新正文、当前文件搜索及已有全局搜索中该文件的结果；切到后台停止监听和轮询，回到前台或切换标签立即检查一次。连续通知按 400 ms 合并，每 30 秒只复核当前文件。自动刷新使用首尾快速校验，未采样中部改写并继续追加可能漏检，F5 可完整重建。“末尾跟随”仅控制正文是否滚到最新一行。持续追加、半行续写、截断和轮转的处理方式及刷新延迟边界见[动态日志文件](doc/dynamic-log-files.md)。
