...
- `state_repository.rs`: SQLite connection/schema ownership plus recent files, pinning, history protection/deletion, workspace recovery, revision-aware session conflict merging, settings, UI values, search preferences/history, filter/color records and database statistics.
- `crates/vclogg-app/src/state_store.rs`: presentation adapter between app models and stable data records; it must not import rusqlite or contain SQL.
