//! Shipped product workflows. Editable copies are installed in the app's private data directory.
...
pub(crate) fn initialize_builtin_skills(
    ...
    for builtin in BUILTINS {
        ...
        if settings.initialized_builtin_skills.contains(&id) {
            continue;
        }
        let leaf = directory.join(builtin.id);
        fs::create_dir_all(&leaf)?;
        let markdown = format!(
            "---\nname: {}\ndescription: {}\n---\n\n{}\n",
            builtin.name, builtin.description, builtin.instructions
        );
        ...
        match options.open(leaf.join("SKILL.md")) {
        ...
        if !settings
            .skills
            .iter()
            .any(|s| s.id == id || s.directory == skill.directory)
        {
            settings.skills.push(skill);
        }
        // Keep tombstones so disabled/removed defaults stay disabled/removed after restarting.
